function execute(grid, input) {
    //grid width and height
    const size = {x:grid.length, y:grid[0].length}
    
    function getTile(x, y) { //get tile of position, return empty if position is OOB
        return (grid[x] ?
            typeof grid[x][y] !== "undefined" ?
                grid[x][y]
            : 16
        : 16)
    }
    ...
    let pc = {rot:0} //pos will be defined when finding the initial starting pos, rot will be initialized
    ...
    if (typeof pc.pos == "undefined") { //if there are no start tiles
        return [false, "Error: there are no start tiles."]
    }
